...
def calculate_std_threshold(tau, p_threshold, m):
    ...
    return tau / (chi_square_value ** 0.5)
...
def train_clone_model(
    ...
    # 输出 victim_feature_loader 中的数据大小
    total_data_size = len(victim_feature_loader.dataset)  # 数据集大小
    total_batches = len(victim_feature_loader)  # 总批次数

    print(f"Total data size in victim_feature_loader: {total_data_size}")
    print(f"Total number of batches in victim_feature_loader: {total_batches}")
