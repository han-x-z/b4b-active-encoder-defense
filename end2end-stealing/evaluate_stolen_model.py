...
def add_common_arguments(parser):
    ...
    parser.add_argument(
        "--losstype", default="infonce", type=str, help="Loss function to use."
    )
    parser.add_argument(
    ...
    parser.add_argument("--mod", default="False", type=str, help="model")
    parser.add_argument("--alpha", default=0.8, type=float, metavar="M", help="Alpha")
    parser.add_argument(
        "--beta", default=80, type=float, metavar="M", help="Beta in %, e.g 80"
    )
    parser.add_argument(
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.modeltype == "stolen":
        ...
        logname = f"testing{args.datasetsteal}{args.num_queries}{args.losstype}{args.dataset}_defence_{args.usedefence}_sybil_{args.n_sybils}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_enhance_attack_{args.enhance_attack}_repeat_times_{args.repeat_times}_query_control_{args.query_control}.log"
        ...
        logging.basicConfig(
            filename=f"{args.pathpre}/testing{args.dataset}.log",
            level=logging.DEBUG,
        )
    ...
    if args.modeltype == "stolen":
        checkpoint = torch.load(
            f"{args.pathpre}/{args.model_to_steal}/checkpoint_imagenet_infonce_{args.num_queries}_defence_{args.usedefence}_sybil_{args.n_sybils}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_enhance_attack_{args.enhance_attack}_repeat_times_{args.repeat_times}_query_control_{args.query_control}.pth.tar",
            map_location="cpu",
        )
        state_dict = checkpoint["state_dict"]
        print("state_dict")
        ...
        for k in list(state_dict.keys()):
            if k.startswith("module.backbone."):
                ...
                if k.startswith("module.predictor.") and not k.startswith(
                    "module.predictor.fc"
                ):
                    # remove prefix
                    new_state_dict[k[len("module.predictor.") :]] = state_dict[k]
    ...
    if args.dataset == "imagenet":
        ...
        train_dataset = datasets.CIFAR10(
            root=f"{args.prefix}/datasets/cifar10",
            train=True,
            download=True,
        ...
        svhn_path = f"{args.prefix}/datasetssets/svhn"

        train_dataset = datasets.SVHN(
            root=svhn_path, split="train", download=True, transform=transform_svhn
        )
        ...
        test_dataset = datasets.SVHN(
            svhn_path, split="test", download=True, transform=transform_svhn
        )
        val_loader = torch.utils.data.DataLoader(
            test_dataset,
            batch_size=args.batch_size,
        ...
        stl_path = f"{args.prefix}/datasetssets/stl10"
        train_dataset = datasets.STL10(
            stl_path, split="train", download=True, transform=transform_stl10
        )
        train_loader = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=args.batch_size,
        ...
        test_dataset = datasets.STL10(
            stl_path, split="test", download=True, transform=transform_stl10
        )
        val_loader = torch.utils.data.DataLoader(
            test_dataset,
        ...
        fmnist_path = f"{args.prefix}/datasetssets/fmnist"
        train_dataset = datasets.FashionMNIST(
            fmnist_path, train=True, download=True, transform=transform_fmnist
        )
        train_loader = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=args.batch_size,
        ...
        test_dataset = datasets.FashionMNIST(
            fmnist_path, train=False, download=True, transform=transform_fmnist
        )
        val_loader = torch.utils.data.DataLoader(
            test_dataset,
            batch_size=2 * args.batch_size,
    ...
    if args.save == "True":
        if args.modeltype == "stolen":
            torch.save(
                model.state_dict(),
                f"{args.pathpre}/SimCLR/downstream/stolen/imagenetenc/{args.datasetsteal}_down_{args.dataset}.pth.tar",
            )
        elif args.modeltype == "victim":
            torch.save(
                model.state_dict(),
                f"{args.pathpre}/SimCLR/downstream/victim/imagenet_down_{args.dataset}_r50.pth.tar",
            )
